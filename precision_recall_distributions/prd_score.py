...
def compute_prd(eval_dist, ref_dist, num_angles=1001, epsilon=1e-10):
  ...
  """
    ...
    if not (epsilon > 0 and epsilon < 0.1):
        raise ValueError('epsilon must be in (0, 0.1] but is %s.' %
                         str(epsilon))
    if not (num_angles >= 3 and num_angles <= 1e6):
        raise ValueError('num_angles must be in [3, 1e6] but is %d.' %
                         num_angles)
...
def _cluster_into_bins(eval_data, ref_data, num_clusters):
  ...
  """
    ...
    eval_bins = np.histogram(eval_labels,
                             bins=num_clusters,
                             range=[0, num_clusters],
                             density=True)[0]
    ref_bins = np.histogram(ref_labels,
                            bins=num_clusters,
                            range=[0, num_clusters],
                            density=True)[0]
...
def compute_prd_from_embedding(eval_data,
                               ref_data,
                               num_clusters=20,
                               num_angles=1001,
                               num_runs=10,
  ...
  """
    ...
    for _ in range(num_runs):
        eval_dist, ref_dist = _cluster_into_bins(eval_data, ref_data,
                                                 num_clusters)
...
def _prd_to_f_beta(precision, recall, beta=1, epsilon=1e-10):
  ...
  """
    ...
    if beta <= 0:
        raise ValueError('Given parameter beta %s must be positive.' %
                         str(beta))
...
def plot(precision_recall_pairs,
         labels=None,
         out_path=None,
  ...
  """
    ...
    if labels is not None and len(labels) != len(precision_recall_pairs):
        raise ValueError('Length of labels %d must be identical to length of '
